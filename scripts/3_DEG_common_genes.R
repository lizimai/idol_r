...
pdf(file=paste0("output/plots/upset_plot_",current_time,".pdf"), width = 8, height = 6) # or other device
upset_plot
dev.off()
...
functional_annotation <- function(overlapping_genes, obiroi_annotation_heiko, id_name, specific_metrics){
  ...
  current_time <- format(Sys.time(), "%Y%m%d_%H%M%S")
  write.csv(gene_with_annotation, paste0("data/processed/DEG_genes_", paste0(specific_metrics, collapse = '_'), "_", current_time,".csv"))
}
