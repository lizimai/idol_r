...
dge <- DGEList(counts = filtered_counts[,2:78], genes = filtered_counts[,1], group=group$PC1)
# Recompute lib sizes
...
save(expr_dist_matrix, behav_dist_matrix, file = paste0("data/processed/dist_matrix_expr_behav_", current_time,".rds"))
...
diff_exp_analysis <- function(counts, group, design_col) {
  ...
  current_time <- format(Sys.time(), "%Y%m%d_%H%M%S")
  pdf(paste0("output/plots/volcano_plot_", design_col, "_", current_time, ".pdf"))
  print(plot)
  dev.off()
  write.csv(sig_genes, paste0("data/processed/significant_genes_", design_col, "_", current_time, ".csv"), row.names=TRUE)
  write.csv(top_genes, paste0("data/processed/top_genes_", design_col, "_", current_time, ".csv"), row.names=TRUE)
...
files <- list.files(path = "data/processed/", pattern = "^significant_genes_", full.names = TRUE)
